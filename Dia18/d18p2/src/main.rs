...
fn main() {
    ...
    let pairs = file.lines().map(|x| x.trim().parse::<Pair>().unwrap()).collect::<Vec<_>>();

    let max_sum = pairs.iter().flat_map(|p1| pairs.iter().filter(move |p| *p != p1).map(|p2| p1.clone() + p2.clone()).map(|x| x.magnitude())).max().unwrap();
