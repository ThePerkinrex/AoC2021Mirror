...
impl Instr {
    ...
    #[allow(dead_code)]
    fn apply<'a, I: Iterator<Item = &'a i64>>(
        &self,
        inp: &mut I,
        x: &mut i64,
        y: &mut i64,
        ...
        match self {
            ...
            Instr::Eql(a, b) => {
                *a.mut_ref(x, y, z, w) = if b.val(*x, *y, *z, *w) == *a.mut_ref(x, y, z, w) {
                    1
                } else {
                    0
                };
...
fn solve(mut inp: [i64; 14], cmds: Vec<Instr>) -> u64 {
    ...
    inp.into_iter()
        .rev()
        .enumerate()
        .fold(0, |r, (i, x)| r + ((x as u64) * 10u64.pow(i as u32)))
}
...
fn main() {
    let instr = include_str!("../../input.txt")
        ...
        .map(|s| s.trim().parse::<Instr>().unwrap())
        .collect::<Vec<_>>();
    let r = solve([9; 14], instr);
    println!(">>> {}", r)
