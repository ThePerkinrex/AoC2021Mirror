        ...
        match self {
            Axis::X(x) => {
                ...
                for y in 0..map.len() {
                    ...
                    for (x_res, x_old) in
                        (0..(map[y].len() - (*x + 1))).map(|i| (*x - i - 1, *x + i + 1))
                    {
                        if map[y][x_old] == 1 {
            ...
            Axis::Y(y) => {
                ...
                for x in 0..map[0].len() {
                    ...
                    for (y_res, y_old) in
                        (0..(map.len() - (*y + 1))).map(|i| (*y - i - 1, *y + i + 1))
                    {
                        if map[y_old][x] == 1 {
...
fn main() {
    ...
    let dots = file
        ...
        .collect::<Vec<_>>();
    let (x_max, y_max) = dots.iter().fold((0, 0), |(x_max, y_max), (x, y)| {
        (x_max.max(*x), y_max.max(*y))
    });

    let mut map = dots.into_iter().map(|(x, y)| (x, y)).fold(
    ...
    let mut instructions = file
        .next()
        .unwrap()
        .lines()
        .map::<Axis, _>(|x| x[11..].parse().unwrap());
